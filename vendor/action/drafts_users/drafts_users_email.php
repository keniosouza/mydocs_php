...
try
{
    ...
    if (count($DraftsUsersValidate->getErrors()) > 0 || count($EmailValidate->getErrors()) > 0)
    {
    ...
    else
    {
        ...
        if (@(int)$resultDraftUsers->draft_user_id > 0)
        {

            /** Gero o nome do arquivo */
            $fileName = 'document/' . $Main->removeAcento($Main->removeMask(str_replace(' ', '_', strtoupper($resultDraftUsers->nickname . '_' . $resultDraftUsers->name)))) . '.pdf';
            ...
            $Email->send($html, $resultDraftUsers, utf8_encode($resultDraftUsers->name), $resultConfiguration->preferences->email, $fileName, $_SESSION['USER_NAME']);
            ...
            if (!empty($resultDraftUsers->history))
            {
            ...
            if ($DraftsUsers->SaveHistory($resultDraftUsers->draft_companies_id, $historyData))
            {
